...
$listar_requerimentos = $db->prepare("SELECT * FROM `atos_oficiais` WHERE `tipo_ato` = 2 ORDER BY `id` DESC");
$listar_requerimentos->execute();
?>
    ...
    <div class="row clearfix">
      <div class="col-xs-12">
        ...
        <span class="breadcrumb-item" itemprop="child" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb">
          <a href="index" itemprop="url"><span itemprop="title">Home </span></a>&gt; <span itemprop="title">Requerimentos</span> </span>
      </div>
    </div>
      ...
      <div class="jumbotron jumbotron-pontilhado jumbotron-busca clearfix">
        <form action="" method="post" name="formulario" id="formulario">
          <div class="col-xs-12 col-sm-8 col-md-2">
            ...
            <input class="input" type="text" id="txtnumero" name="txtnumero" placeholder="N° ato" />
          </div>
          <div class="col-xs-12 col-sm-8 col-md-2">
    ...
    <div class="col-xs-12 vereadores-mandato">
      <div class="table-responsive">
        <table id="tableRequerimentos" class="display col-xs-12 col-sm-12 dataTable no-footer" style="width:100%">
          ...
          <tbody>
            <?php foreach ($listar_requerimentos as $key => $value) { ?>
              <tr>
                <td>
                  <a href="<?= $value['arquivo'] ?>" target="_blank" class="link-table clearfix">
                    <strong class="vereador-nome"> <?=$value['id'] ?> REQUERIMENTO - <?= $value['titulo'] ?> </strong>
                    <p align="justify"> <?= date('d/m/Y', strtotime($value['data_cadastro'])) ?> - <?= $value['descricao'] ?> </p>
                  </a>
                </td>
              </tr>
            <?php } ?>
